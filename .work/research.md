...
✔ family complete: `spatial-{panner, widener, haas, autopan, crossfeed, midside, channelsplit, delay, microshift, surround, binaural}` — 18 tests (M/S roundtrip identity, sample-exact delay, microshift ±cents verified, 5.1 matrix content checks). FFmpeg + H3000 parity closed. binaural (2026-07-10) = Brown-Duda 1998 structural HRTF — dataless (no HRIR assets): two-branch spherical ITD (cross-correlation-verified ±1 sample), one-pole-one-zero head shadow (paper constants verified: αmin 0.1, θmin 150°), 1/r distance, self-smoothed azimuth; pinna elevation omitted with citation (per-subject-fit).
...
✔ `spectral-{centroid, spread, flatness, rolloff, flux, slope, crest, mfcc, ltas, edit}` — analytic identities (Peeters 2004 / aspectralstats; MFCC gain-invariance per DCT property; LTAS Welch; edit = COLA STFT region gains with reconstruction + band-kill verified). ✔ family complete: + `spectral-{freeze, contrast, harmonics, cqt, target}` — 31 tests total (freeze sustains, contrast tone≫noise, harmonics matches analytic saw T1=0.645, CQT exact octave spacing). target (2026-07-10) = **the target-curve library** (todo § Auto-Chain "Still to build" → shipped): speech = Byrne et al. 1994 LTASS (JASA 96(4) Table II, transcribed + cited), music = Pestana et al. 2013 (−5 dB/oct 100–4000 Hz, cited), pink analytic, voice-music convention; `deviation()` (band-limited, mean-normalized, ⅓-oct smoothed, ±clamp) + `smooth()` = the adaptive-EQ / Match-by-Reference core; manifest-less by design (pure data+math).
