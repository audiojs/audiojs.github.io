...
- [x] Extend local packed-consumer/example/type checks to the selected DSP path and its dependencies; retain family signal tests. `test:types` now runs the same effect API regressions from installed tarballs, executes dynamics/EQ/effect README examples, and verifies actual delay/ping-pong AudioWorklets with imported helpers in Chromium/Firefox/WebKit at 44.1/48/96 kHz. Public-registry verification remains a release gate.
- [~] Unify documented expectations: units, mutation, state/reset, streaming, modulation, latency/tails and runtimes. Dynamics/EQ/effect now document these distinctions. Remaining factory/automation limits must be resolved before recommending the wider catalog.
- [x] **Effect defaults and delay state.** All 24 mismatched kernels now accept omitted/undefined options; 25 PCM function types preserve Float32Array/Float64Array returns. Delay and ping-pong use a causal one-sample minimum, reset on effective-length changes and validate time/rate. Stereo length mismatches reject; mixer handles empty/equal-length inputs explicitly. Chorus/flanger zero-length rings and multitap's default-cache allocation are fixed. Delay wrappers drain disconnected tails, isolate channels/instances and estimate tail decay from settings. Full family: 130 tests; identical API regressions also run from tarballs.
- [ ] **Next: generated factory types must match the processor contract.** `toWam(delay)` currently fails strict TypeScript because generated `Ctx` requires `maxChannels`, absent from `ProcessorContext`; its params/context typing also needs alignment. Fix `compile/tools/dts.js` at the source, regenerate affected declarations, and check direct factory → adapter composition in NodeNext/bundler consumers without casts or new runtime dependencies.
- [ ] Extend disconnected-input/tail and automation coverage beyond delay/ping-pong. Many sibling wrappers return immediately on absent input; distinguish stateless/batch processors from processors with pending tails. Audit zero-time multitap semantics and retained-history behavior on modulation-ring size changes. Test time-change restart, feedback automation, sample-rate/layout changes, and declared latency/tails before widening the supported path.
...
6. [x] **2026-07-10 component wave published** (all app components except `audio-chain` now shipped): NEW `@audio/dynamics-unlimit` 0.1.0 (deficit-mode classical de-limiter — Ozone-Unlimiter counterpart, 3-gate detector) · `@audio/spatial-binaural` 0.1.0 (Brown-Duda structural HRTF, dataless, paper-verified constants) · `@audio/spectral-target` 1.0.0 (target-curve library: Byrne/Pestana-cited + deviation/smooth — Auto-Chain data layer done) · umbrellas dynamics 0.2.4, spatial 1.0.1, spectral 1.2.1 (+ envelope 0.1.2, multiband 0.4.1 d.ts-regen republishes) · `audio@2.6.3` (registry: unlimit, binaural). Suites: dynamics 56 · spatial 18(+4) · spectral 31 · engine 669/669 (one meter-playback flake proven environmental via standalone probe: 612 ms, rms 0.707 exact). Committed + pushed.
...
**The module layer is DONE** (2026-07-10): every DSP stage below is a shipped package, the target-curve data is `@audio/spectral-target`, the orchestrator is `@audio/chain` (engine op `auto`). What's missing is only the pages themselves. In ship order:
...
* [x] **`@audio/chain` 0.1.0 shipped 2026-07-10** (decided same day; was "audio-chain" — unscoped names deprecated in July, and it IS an atom): infrastructure-register sibling of compile/host/wam, repo `audiojs/chain` created. `analyze()` (LUFS/TP/LRA, min-statistics noise PSD, LTAS, sibilance, mains-hum Goertzel, clipping, denoise-detect clicks, VAD) → `plan()` (adaptive inclusion, cited `why` per stage; speech/music/voice-music via `spectral-target`; `{reference}` = Match-by-Reference) → `apply()` (single LUFS-refinement trim, re-limited) + `code()` JS-recipe export; manifest: `auto` processor (streaming:false one-knob enhance) + `chain` stat (recipe w/o processing); registry `auto:` in `audio@2.6.5`. Measured: hum −33 dB, noise floor −22.5 dB, LUFS ±1e-9, reference tilt gap −59.8%; 9/9 tests. **Every component of the flagship tools now exists — remaining work is the pages.**
...
* [x] **Speech renderer and waveform navigation:** replace spectral pitch shifting in the editor with pinned WORLD reference synthesis; use smooth, shape-preserving pitch curves; add visible zoom, keyboard/Alt-scroll zoom and position panning. Dynamic vowel coherence and 8–96 kHz regressions added.
* [x] **Continuous speech pitch tracking:** replace independent YIN frames with WORLD DIO/StoneMask at 5 ms. Pin the sample's false-unvoiced gap at 1.745–1.765 s with an end-to-end regression for rendered pitch, plus true consonant/silence preservation. See [diagnosis and quality sequence](prosody-quality.md).
* [ ] **Speech correction listening gate:** first collect varied real voices and matched-loudness reference edits; then separate voiced syllables, slow intonation, modulation and transitions; validate consonant/breath boundaries and compare pulse-synchronous synthesis against WORLD on that same corpus before claiming high-end quality. Release the corrected spectral/shift packages to npm separately.
...
### 2026-07-10 — org audit → staging wave (full record: audit.md, deleted; in git history)

- [x] **Audit**: 337 packages / 54 repos swept (mechanical inventory + registry drift + cold-install smoke + 6 parallel per-family API reviews). Verdict: 'one focused wave away from staging' — all four fix waves executed same day, then funding + docs waves on top.
- [x] **Fix waves**: Correctness on npm: multiband/envelope rate bug (root-cause dual-read + 44.1k/96k invariance test), umbrella stale ranges, synth `(freq, opts)` unification (fm/modal 0.2, drum 2.0, noise 2.0 seconds+colors-main, poly voiceOpts + real voice tests), stretch channel-arrays ×9, filter manifest-defaults→kernel ×5 + biquad manifest fc/Q 2.0, WAM restart+streaming:false refusal (22 tests), `audio@2.6.4` (a-weighting→weighting-a.response, batch suite into test:all, +5 hosted tests). Truth: 34 atom + 7 umbrella stale-"planned" READMEs rewritten from kernels (examples executed); effect §Dynamics → pointer; filter strands → pointers + biquad section; weighting k/riaa gained `.response`. Conventions: CONTRACT §Conventions (fs/sampleRate boundary, units per class, kernel-is-truth); chorus/flanger delay→seconds + vibrato depth→0..1 (majors); dehum q→Q. Dedupe: digital-filter→window-function (2.4.1 bit-identical), biquad `cascadeMagnitude` (7 weighting copies gone), amp/saturate loops→biquad (0.0 diff), speech-lpc→@audio/lpc (2.7e-15), note ×4, stft→@audio/window, deesser shim. Strategy: audio-decode un-deprecated + 3.11.2 maintained alias; digital-filter WIP shipped **non-breaking as 2.4.0** (sosfilt_zi alias, poles/type2 kept, dynamicSmoothing maxFc throws, levinson sign flagged); audio-buffer-utils deprecated. Plumbing: metadata ×234 fields org-wide, CI test.yml ×25, gh descriptions ×22, release.mjs +5 extra repos + STALE-range class.
- [x] **Funding wave**: `funding` field ×315 published (npm fund resolves org-wide) · FUNDING.yml ×47 · audiojs/.github org profile ('Open Audio Stack': live badges, Atoms/Verified/Roadmap-with-jz, snippets executed cold before publishing) · sponsors kit `.work/sponsors.md` (listing + tiers + account-holder enablement steps — Sponsors URL 404s until enabled).
- [x] **Docs wave**: per-atom READMEs 178→0 + kernel d.ts 216→0 (denoise pattern, tsc-strict; atomdocs.mjs gained SECTION_ALIAS/FN_NAME; umbrella doc bugs caught: wah depth octaves, oberheim phantom drive, defeedback analyzer kc) · weighting audio.js manifests ×7 (curves smoke-verified) · host packages got real index.d.ts (audit's broken types fields).
- [x] **CI green org-wide**: lockfile syncs (npm ci class), wam glob + file:-dep + emit race, engine navigator restores ×2 + batch npm imports, node-gyp matrix (VS2026 needs gyp≥12 needs node≥22; binaries now build on 7 mic legs), host-clap _Thread_local→MSVC (last red leg), mic pacing CI-gated.
- [x] **Engine 2.6.7/2.6.8** (both bugs found by cold-running the org-profile snippets): whole-render atoms on still-decoding file sources rendered empty/crashed (LOAD decode-wait + save live-guard + plan empty-timeline tolerance) · encoder constructed before metadata (mp3 of a mono file threw on the README's own flagship line). Regressions locked.
- [x] **Tooling**: release.mjs — +7 extra repos, STALE dep-range class (the umbrella bug class), package.json joins the content digest version-normalized (funding wave produced 2 DIRTY of 315 before), main-in-pack publish guard (caught 3 empty mic binaries → deprecated with pointers).
- Remaining holds live in Now §3 (mic CI-runner binaries, pcm-convert OTP, decode wasm WIP) + Next (engine-native stat two-tier + auditory/measure/pitch/beat manifest exemptions — documented designs).
