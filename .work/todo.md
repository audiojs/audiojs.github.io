...
- [~] Extend packed-consumer/example/type checks to the selected DSP path and its dependencies; retain family signal tests. Dynamics/EQ now add executable installed README examples, signal/state checks and complete umbrella exports to `test:types` (210 local tarballs). Next: exercise an actual DSP leaf through the browser adapter, including its imported helpers.
- [~] Unify the documented expectations: units, input/output and mutation, state/reset, streaming suitability, modulation, latency/tails and supported runtimes. Dynamics/EQ now document ownership, rate keys, units and streaming limits; 14 missing typed exports and five omitted EQ declaration files are repaired. Fill missing behavior evidence before marking packages recommended.
- [ ] **Next API repair: effect defaults and delay state.** Direct calls to 24 effect leaves throw when params is omitted, although their declarations make it optional. Test and unify a default-options contract across these siblings. Delay additionally produces NaN at zero/sub-sample time and retains stale ring history when its length shrinks, contrary to its declaration. Pin smallest/empty inputs, delay-boundary splits, repeated signals, independent instances and time changes before using delay as the supported effect. Validate its fixed 8-second tail metadata against long/high-feedback settings.
