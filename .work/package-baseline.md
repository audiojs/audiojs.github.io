...
2. **Continue the installed-package check on the supported DSP path.** Dynamics/EQ now have executable first README examples, strict types and signal/state assertions in a fresh tarball consumer. Next, extend the browser adapter's installed checks to a real DSP leaf with imported helpers, beyond the self-contained compiler examples. Keep family source tests. This is the protection against workspace-only success and empty npm releases.
3. **Repair the next observed API mismatch, then finish the behavior inventory.** Twenty-four effect leaves throw on omitted params despite optional declarations. Test and unify that contract across the siblings. Delay's zero/sub-sample time yields NaN; shortening the delay retains stale ring history rather than resetting as its types claim. Verify these boundaries and tail metadata before recommending it. Continue units, state/reset, modulation and target-limit documentation from the dynamics/EQ pattern. Native release remains gated on JZ v1.
