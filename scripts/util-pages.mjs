...
writeFileSync(`${ROOT}util/libs.js`, `// Generated by scripts/util-pages.mjs — do not edit; run \`npm run build\`.
export const DECODE = '${src('@audio/decode', '3.15.0')}'
export const ENCODE = '${src('@audio/encode', '1.6.5')}'
export const META = 'https://esm.sh/@audio/decode@3.14.1/meta'
export const TYPE = '${src('audio-type', '2.7.0')}'
export const MIC = '${src('@audio/mic', '1.1.2')}'
`)
...
export const pages = [
  {
    slug: 'extract-audio', name: 'Extract audio from video', short: 'Video → MP3, WAV, FLAC, M4A, ALAC, OGG, Opus, AAC',
...
`,
    tool: { formats: ['mp3', 'wav', 'flac', 'm4a', 'alac', 'ogg', 'opus', 'aac'] },
    ...
    faq: [
      ...
      ['Which video formats are supported?', 'Video: MP4, M4V, MOV, 3GP, WebM, MKV and AVI with AAC, AC-3, E-AC-3 (Dolby Digital Plus), DTS, MP3, Opus, Vorbis, FLAC, ALAC or PCM audio. Surround tracks keep their channels. Audio: MP3, WAV, FLAC, OGG, Opus, AAC, M4A, AIFF, CAF, WMA and AMR.'],
      ['Which output formats can I save?', 'MP3, WAV, FLAC, M4A (AAC), ALAC, OGG Vorbis, Opus and AAC. WAV, FLAC and ALAC are lossless; MP3, M4A, OGG, Opus and AAC are compressed. M4A with AAC needs a browser with WebCodecs (Chrome, Edge, Safari).'],
    ...
    seo: `
      ...
      <ul>
        <li><strong>Video</strong>: MP4, M4V, MOV, 3GP, WebM, MKV and AVI carrying AAC, AC-3, E-AC-3, DTS, MP3, Opus, Vorbis, FLAC, ALAC or PCM audio. 5.1 tracks keep all six channels.</li>
      ...
      <p>When the in-page decoder cannot read a file (TrueHD) it hands the bytes to your browser's own decoder, and as a last resort plays the file silently through the audio graph and records the output in real time. To swap or strip a video's soundtrack without touching the picture, use <a href="/util/video-audio/">replace or remove the audio track</a>.</p>
      <h3>Common uses</h3>
      <ul>
  ...
  },
  {
    ...
    title: 'Audio converter: MP3, WAV, FLAC, M4A, ALAC, OGG, Opus, AAC, WavPack, free in your browser',
    description: 'Convert audio between MP3, WAV, FLAC, M4A (AAC, ALAC), WavPack, OGG Vorbis, Opus, AAC, AIFF, CAF, WebM and QOA in your browser. Reads APE, TTA, Musepack, DSD, E-AC-3 and MOD/XM/S3M/IT too. Tags carry over. No upload, no size limit.',
    ...
    body: drop('Drop an audio file here', 'audio/*,video/*,.m4a,.flac,.opus,.wma,.amr,.caf,.aiff,.ac3,.eac3,.ec3,.dts,.qoa,.wv,.ape,.tta,.mpc,.dsf,.dff,.mod,.xm,.s3m,.it') + `
    <form class="opts" id="opts">
      ...
      <label data-for="mp3 aac opus webm m4a">Bitrate <select name="bitrate"><option value="64">64 kbps</option><option value="96">96 kbps</option><option value="128">128 kbps</option><option value="192">192 kbps</option><option value="256" selected>256 kbps</option><option value="320">320 kbps</option></select></label>
...
`,
    tool: {},
    script: `
    const BITRATES = { mp3: [96, 128, 192, 256, 320], aac: [64, 96, 128, 192, 256], m4a: [64, 96, 128, 192, 256], opus: [32, 64, 96, 128, 160, 192], webm: [32, 64, 96, 128, 160, 192] }
    ...
    faq: [
      ['Which formats can I convert from?', 'MP3, WAV, FLAC, OGG Vorbis, Opus, AAC, M4A, ALAC, AIFF, CAF, WMA, AMR, AC-3, E-AC-3, DTS, QOA, WavPack, APE (Monkey\'s Audio), TTA, Musepack, DSD (DSF, DFF) and tracker modules (MOD, XM, S3M, IT), plus the audio track of MP4, MOV, MKV, WebM and AVI video.'],
      ['Which formats can I convert to?', 'MP3, WAV, FLAC, M4A (AAC), ALAC (Apple Lossless in M4A), WavPack, OGG Vorbis, Opus, AAC, AIFF, CAF, WebM (Opus) and QOA. Lossy formats take a bitrate (or a quality level for Vorbis); PCM formats take a bit depth of 16, 24 or 32-bit float. M4A with AAC needs a browser with WebCodecs (Chrome, Edge, Safari); ALAC and the rest work everywhere.'],
      ['Do the tags survive?', 'Title, artist, album, year, cover art and the other common fields are read from MP3, FLAC, WAV, M4A, Ogg and Opus sources and written into WAV, MP3, FLAC, AIFF, OGG, Opus, M4A, ALAC and WavPack outputs.'],
      ...
      ['Is the conversion lossless?', 'WAV, FLAC, ALAC, WavPack, AIFF and CAF preserve every decoded sample at the chosen bit depth. Converting between two lossy formats re-compresses; keep a lossless master when you can.'],
      ['What about DSD and tracker modules?', 'DSF and DFF files are converted from 1-bit DSD to PCM with a 100 dB-stopband decimation filter (DSD64 becomes 88.2 kHz). MOD, XM, S3M and IT modules are rendered with libopenmpt at 48 kHz stereo, so a chiptune becomes an MP3 or FLAC.'],
    ],
    seo: `
      ...
      <ul>
        ...
        <li><strong>M4A</strong> (AAC) is what iPhones, iTunes and video editors expect; <strong>ALAC</strong> is Apple Lossless in the same container, encoded with a port of Apple's own encoder. <strong>WavPack</strong> is lossless with the widest bit-depth support, common in archives.</li>
...
// one module per page in scripts/pages/ — `order` (default 50) sorts among the inline pages above (10, 20, …)
pages.forEach((p, i) => p.order ??= (i + 1) * 10)
for (const f of readdirSync(`${ROOT}scripts/pages`).filter(f => f.endsWith('.mjs') && !f.endsWith('.test.mjs')).sort()) pages.push({ order: 50, ...(await import(`./pages/${f}`)).default })
...
const index = () => `${head({ slug: '', title: 'Free audio utilities that run in your browser', name: 'Audio utilities', description: 'Extract audio from video, replace or remove a video soundtrack, convert formats, transcribe speech to subtitles, measure loudness, remove noise and silence, fix wow and flutter, check whether a FLAC is really lossless, find key and BPM, remove vocals, change pitch and tempo, render MIDI, transcribe audio to MIDI and sheet music, tune, record, view spectrograms and measure your room. Every tool runs in the browser on audiojs open-source packages; nothing is uploaded.', repo: 'https://github.com/audiojs', faq: [] }).replace(/util\/\//g, 'util/')}
...
// the homepage's Utilities list is the same pages, kept in sync between its markers
{
  const home = readFileSync(`${ROOT}index.html`, 'utf8')
  const open = home.indexOf('<!-- utilities:'), close = home.indexOf('<!-- /utilities -->')
  if (open < 0 || close < 0) throw Error('index.html: utilities markers not found')
  const items = pages.map(p => `\n        <li><a href="/util/${p.slug}/"><div class="t-name">${esc(p.name)}</div><div class="t-desc">${esc(p.short)}</div></a></li>`).join('')
  writeFileSync(`${ROOT}index.html`, home.slice(0, home.indexOf('-->', open) + 3) + items + '\n        ' + home.slice(close))
}
