...
try {
  for (const engine of (process.argv.length > 2 ? process.argv.slice(2) : ['chromium', 'firefox', 'webkit'])) {
    ...
    try {
      ...
      assert.equal(await page.locator('#view-position').isDisabled(), true)
      const selected = await page.locator('#end').inputValue()
      await page.click('#zoom-in')
      assert.match(await page.locator('#view-range').textContent(), /2.0×/)
      assert.equal(await page.locator('#render').isDisabled(), true, 'view changes are not audio edits')
      await page.locator('#view-position').fill('1')
      assert.match(await page.locator('#view-range').textContent(), /3.09–6.18 s/)
      assert.equal(await page.locator('#end').inputValue(), selected, 'panning preserves selection')
      await page.locator('#curve').focus(); await page.keyboard.press('+')
      assert.match(await page.locator('#view-range').textContent(), /4.0×/)
      await page.keyboard.press('-')
      assert.match(await page.locator('#view-range').textContent(), /2.0×/)
      await page.keyboard.press('0')
      assert.equal(await page.locator('.target').getAttribute('d'), before)
      await page.locator('#curve').dispatchEvent('wheel', { deltaY: -100, altKey: true, clientX: 400, clientY: 500 })
      assert.equal(await page.locator('#zoom-less').isEnabled(), true, 'Alt + wheel zooms the waveform')
      ...
      await page.click('#demo')
      await page.waitForFunction(() => document.querySelector('#status').textContent.startsWith('Ready.'))
      assert.match(await page.locator('#view-range').textContent(), /1.0×/)
      assert.equal(await page.locator('#view-position').isDisabled(), true)
