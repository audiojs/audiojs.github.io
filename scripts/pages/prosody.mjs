...
export default {
  ...
  faq: [
    ...
    ['Will it preserve the voice?', 'The WORLD speech engine rebuilds each edited vowel or voiced phrase from its own vocal-tract spectrum and breathiness, at the new pitch and timing, and restores its original loudness contour. Consonants, silence and untouched phrases keep their original samples. Edits can still sound processed, especially with creaky speech or large changes. Compare with the original before saving.'],
  ...
  seo: `<h2>Edit delivery, one phrase at a time</h2><p>Select a word or phrase, reduce excessive pitch variation, shift its emphasis or adjust its duration. Pinch to zoom into the waveform and drag on the plot to select a phrase.</p><p>Pitch and timing edits use the <a href="https://github.com/mmorise/World">WORLD reference speech engine</a>, compiled to WebAssembly and run locally. Harvest tracks pitch continuously across each phrase. Every edited voiced phrase is rebuilt whole at the new pitch and duration with its original loudness contour; unvoiced audio in a retimed span is stretched with WSOLA and consonant bursts keep their length. WAV export contains audio only, without source tags.</p>`,
}
