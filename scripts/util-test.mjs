// End-to-end tests for every utility page across browser engines.
// Usage: node scripts/util-test.mjs [chromium] [firefox] [webkit]
...
const STUB = `
  ...
  navigator.mediaDevices.getUserMedia = async () => { const ctx = new AudioContext({ sampleRate: 48000 }); const dest = ctx.createMediaStreamDestination(); await ctx.resume(); await window.__micSource(ctx, dest); return dest.stream }
